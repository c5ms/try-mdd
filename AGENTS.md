...
用户确认 → 是否更新到领域模型目录？
  ↓
更新设计 → design/domain/{domain}.md（最终设计）
```
...
design/
  ...
  model.md                # 领域模型汇总（统一术语表、领域模型图、演进记录）
  domain/
...
|------|----------|----------|
| `solution/` | 评审阶段 | 待确认方案 |
| `design/domain/*.md` | 确认阶段 | 最终设计结果（平铺） |
| `design/model.md` | 确认阶段 | 领域模型汇总 |
| `design/todo.md` | 设计全程 | 待定问题追踪 |
...
| `mdd-apply` | 搜索方案 → 用户确认 → 执行更新 → 更新汇总 → 更新待定问题 → 清理方案 | `.opencode/skills/mdd-apply/SKILL.md` |
...
| 批量应用方案 | `/m-apply` | 搜索方案 → 用户确认 → 执行更新 → 更新汇总 → 更新待定问题 → 清理方案 |
